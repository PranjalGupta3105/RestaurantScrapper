...
async function Scrapper(restaurantName){

    console.log('Able to Come here : launchPuppeter');
    
    puppeteerInstance.launch().then(async browser =>{
        const page = await browser.newPage();
        await page.goto('https://maps.google.com');
        await page.screenshot({path: 'maps.png'})


        // --- Find Search box element, insert the text and search the element

        let searchElement  = page.focus('#searchboxinput');
        page.keyboard.type(restaurantName);
        await page.screenshot({path: 'beforesearch.png'});
        let searchSelector = '#searchbox-searchbutton';
        // let searchSelector2 = '*[@id="searchbox-searchbutton"]';

        await page.evaluate((searchSelector) => 
        document.querySelector(searchSelector).click(), searchSelector);
        await page.waitFor(60*1*1000)
        await page.screenshot({path: 'searched.png'});

        let searchResultSelector = '#pane > div > div.widget-pane-content.scrollable-y > div > div > div.section-layout.section-scrollbox.scrollable-y.scrollable-show.section-layout-flex-vertical > div.section-layout.section-scrollbox.scrollable-y.scrollable-show.section-layout-flex-vertical > div:nth-child(1) > div.section-result-content';
        await page.evaluate((searchResultSelector) => 
        document.querySelector(searchResultSelector).click(), searchResultSelector);
        await page.waitFor(60*1*1000)
        await page.screenshot({path: 'finalResult.png'});

        let descriptionElement = '#pane > div > div.widget-pane-content.scrollable-y > div > div > jsl > button > div > div:nth-child(1) > div.section-editorial-quote.section-editorial-divider > span'
        let descElement = await page.$(descriptionElement);
        let descriptionText = await page.evaluate(descElement => descElement.textContent, descElement);
        console.log(descriptionText);
        

        
        // await page.xpath(searchSelector2).click();
        // await page.screenshot({path: 'searched.png'});
...
// async function getElementSetParameterAndSearch(pageInstance, restaurantName='burger king'){

//     console.log('Able to Come here ');

//     let searchElement  = this.pageInstance.focus('#searchboxinput');
//     this.pageInstance.keyboard.type(restaurantName)
//     await pageInstance.screenshot({path: 'search.png'})
// }
...
async function resultantFunction(){

    let pageInstance = await Scrapper('burger king');
